...
def run(matrix_sizes: Optional[list[int]] = None) -> tuple[list,list,list]:
    ...
    for size in matrix_sizes:
        rt_diff, p_diff = compare(size)
        rt_diffs.append(rt_diff)
        p_diffs.append(p_diff)

    return matrix_sizes, rt_diffs, p_diffs

def save_to_csv(matrix_sizes, rt_diffs, p_diffs, filename):
    """Saves the matrix sizes, runtime differences, and percentage differences to a CSV file."""
    header = ['Matrix Size (NxN)', 'Runtime Difference (seconds)', 'Percentage Difference (%)']
    rows = zip(matrix_sizes, rt_diffs, [100 * p for p in p_diffs])  # Combine all lists into rows
...
def plot(sizes, rt_diffs, p_diffs):
